...
## api
THIS API IS UNDER DEVELOPMENT AND SUBJECT TO ROUTINE AND EVEN CONSTANT BREAKING CHANGES.

### elem

### attributes
the attributes on an `elem` may be set by passing its constructor the following keyword arguments.  by convention, in it good practice to lay them out in the same order. any attribute may accept the global values `:initial` and `:inherit`.
...
**aligments**: `:ah :av`. the align horizontal and align vertical values may be one of the following corresponding keywords.
  * **horizontal keywords**.
    * `:left` (default). align children to the left.
    * `:center`. align children to the center.
    * `:right`. align children to the right.
    * `:justify`. evenly space children to fill all but last line.
  * **vertical keywords**:
    * `:top` (default). align children to the top.
    * `:middle`. align children to the middle.
