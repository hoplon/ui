...
(defn mkcalc [f op type-fn]
  (fn [& vs]
    ...
    (if (every? number? vs) (apply f vs) (Calc. op (type-fn vs)))))

(defn num->px [n] (if (number? n) (Pixels. n) n))
...
(def + (mkcalc clojure.core/+ "+" (partial mapv num->px)))
(def - (mkcalc clojure.core/- "-" (partial mapv num->px)))
(def * (mkcalc clojure.core/* "*" #(update % [0] num->px)))
(def / (mkcalc clojure.core// "/" #(update % [0] num->px)))
