(ns hoplon.ui.transforms)

;;; utils ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defn pow [x e] (js/Math.pow x e))
(defn sin [x] (js/Math.sin x))
(defn cos [x] (js/Math.cos x))
(def pi js/Math.PI)

;;; formulas ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;-- linear --------------------------------------------------------------------;

(defn linear
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range. implemented by calculating
   the slope between the (d1, r1) and (d2, r2) and substituting it into a linear
   equation in point-slope form."
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (/ (* c (- x d1)) d) r1))))

;-- quadratic -----------------------------------------------------------------;

(defn quadratic-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a positive quadratic
   function"
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (* c (pow (/ (- x d1) d) 2)) r1))))

(defn quadratic-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a negative quadratic
   function"
  [[d1 d2] [r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
     (fn [x] (+ (* (- c)  (/ (- x d1) d) (- (/ (- x d1) d) 2)) r1))))

(defn quadratic-in-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a negative quadratic
   function"
  [[d1 d2] [r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
      (fn [x]
        (if (< x (+ d1 (/ d 2)))
            (+ (* (/ c 2) (pow (/ (- x d1) (/ d 2))2)) r1)
            (+ (* (- (/ c 2)) (- (* (- (/ (- x d1) (/ d 2)) 1) (- (- (/ (- x d1) (/ d 2)) 1) 2)) 1)) r1)))))

;-- cubic ---------------------------------------------------------------------;

(defn cubic-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 3rd degree polynomial
   function"
  [[d1 d2] [r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
     (fn [x] (+ (* c (pow (/ (- x d1) d) 3)) r1))))

(defn cubic-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 3rd degree polynomial
   function"
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (* c (+ (pow (- (/ (- x d1) d) 1) 3) 1)) r1))))

(defn cubic-in-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a negative quadratic
   function"
  [[d1 d2] [r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
      (fn [x]
        (if (< x (+ d1 (/ d 2)))
            (+ (* (/ c 2) (pow (/ (- x d1) (/ d 2)) 3)) r1)
            (+ (* (/ c 2) (+ (pow (- (/ (- x d1) (/ d 2)) 2) 3) 2)) r1)))))
;-- quartic -------------------------------------------------------------------;

(defn quartic-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 4th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
     (fn [x] (+ (* c (pow (/ (- x d1) d) 4)) r1))))

(defn quartic-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 4th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (* (- c) (- (pow (- (/ (- x d1) d) 1) 4) 1)) r1))))

(defn quartic-in-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 4th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
      (fn [x]
        (if (< x (+ d1 (/ d 2)))
            (+ (* (/ c 2) (pow (/ (- x d1) (/ d 2)) 4)) r1)
            (+ (* (- (/ c 2)) (- (pow (- (/ (- x d1) (/ d 2)) 2) 4) 2)) r1)))))

;-- quintic -------------------------------------------------------------------;

(defn quintic-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 5th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (* c (pow (/ (- x d1) d) 5)) r1))))

(defn quintic-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 5th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
  (let [c (- r2 r1)
        d (- d2 d1)]
    (fn [x] (+ (* c (+ (pow (- (/ (- x d1) d) 1) 5) 1)) r1))))

(defn quintic-in-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a 4th degree polynomial
   function"
  [[d1 d2] [r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
      (fn [x]
        (if (< x (+ d1 (/ d 2)))
            (+ (* (/ c 2) (pow (/ (- x d1) (/ d 2)) 5)) r1)
            (+ (* (/ c 2) (+ (pow (- (/ (- x d1) (/ d 2)) 2) 5) 2)) r1)))))

;-- sine ----------------------------------------------------------------------;

(defn sine-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a sine function"
  [[d1 d2][r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
      (fn [x] (+ (* (- c) (cos (* (/ (- x d1) d) (/ pi 2)))) c r1))))

(defn sine-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a sine function"
  [[d1 d2][r1 r2]]
    (let [c (- r2 r1)
           d (- d2 d1)]
      (fn [x] (+ (* c (sin (* (/ (- x d1) d) (/ pi 2)))) r1))))

(defn sine-in-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a sine function"
  [[d1 d2][r1 r2]]
    (let [c (- r2 r1)
         d (- d2 d1)]
      (fn [x] (+ (* (/ (- c) 2) (- (cos (* (/ (- x d1) d) pi)) 1)) r1))))

;-- exponential ---------------------------------------------------------------;

(defn exp-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a exponential function"
  [[d1 d2][r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
      (fn [x] (+ (* c (pow 2 (* 10 (- (/ (- x d1) d) 1)))) r1))))

(defn exp-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a exponential function"
  [[d1 d2][r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
     (fn [x] (+ (* c (+ (- (pow 2 (* (- 10) (/ (- x d1) d)))) 1)) r1))))

(defn exp-in-out
  "given a domain and a range, return a function that will translate any given
  value in that domain to the corresponding range using a 4th degree polynomial
  function"
 [[d1 d2] [r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
      (fn [x]
       (if (< x (+ d1 (/ d 2)))
           (+ (* (/ c 2) (pow 2 (* 10 (- (/ (- x d1) (/ d 2))1)))) r1)
           (+ (* (/ c 2) (+ (- (pow 2 (* (- 10) (- (/ (- x d1) (/ d 2)) 1))))2)) r1)))))


;-- circular ------------------------------------------------------------------;

(defn circ-in
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a circular function"
  [[d1 d2][r1 r2]]
    (let [c (- r2 r1)
          d (- d2 d1)]
       (fn [x] (+ (* (- c) (- (pow (- 1 (pow (/ (- x d1) d) 2)) 0.5) 1)) r1))))

(defn circ-out
  "given a domain and a range, return a function that will translate any given
   value in that domain to the corresponding range using a circular function"
   [[d1 d2][r1 r2]]
     (let [c (- r2 r1)
           d (- d2 d1)]
        (fn [x]
          (if (< x d1)
              r1
              (+ (* c (pow (- 1 (pow (- (/ (- x d1) d) 1) 2)) 0.5)) r1)))))

(defn circ-in-out
  "given a domain and a range, return a function that will translate any given
  value in that domain to the corresponding range using a circular
  function"
 [[d1 d2] [r1 r2]]
   (let [c (- r2 r1)
         d (- d2 d1)]
      (fn [x]
        (if (< x d1)
            r1
            (if (< x (+ d1 (/ d 2)))
                (+ (* (/ (- c) 2) (- (pow (- 1 (pow (/ (- x d1) (/ d 2)) 2)) 0.5)1)) r1)
                (+ (* (/ c 2) (+ (pow (- 1 (pow (- (/ (- x d1) (/ d 2)) 2) 2)) 0.5) 1)) r1))))))
