...
(extend-type number
  ...
  (-assoc [color k v]
    (case k
      ...
      :b  (byte-set color 2 v)
      :a  (byte-set color 0 v)
      ; :h  (hcl v           (chroma color)         (value color)) ;todo
      :c  (hcl (hue color) v                      (luma  color))
      :y  (hcl (hue color) (chroma v)             v)
      :h  (hsl v           (saturation-hsl color) (lightness color))
      :s  (hsl (hue color) v                      (lightness v))
      :l  (hsl (hue color) (saturation-hsl v)     v)
      :sv (hsv (hue color) v                      (value color))
      :v  (hsv (hue color) (saturation-hsv color) v)
      :si (hsi (hue color) v                      (intensity v))
      :i  (hsi (hue color) (saturation-hsi v)     v)))
  IFn
  (-invoke [color k]
    (-lookup color k)))
