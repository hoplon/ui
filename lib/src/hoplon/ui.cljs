...
(defn pointable [ctor] ; _p_ointer
  ...
  (fn [{:keys [m me ms click draggable drop] :as attrs} elems]
    {:pre [(v/cursors? m) (v/events? me) (v/callbacks? click drop)]}
    (with-let [e (ctor attrs elems)]
      (let [ms (cell= (if ms :text :none))]
        (bind-in! e [mid .-draggable]                draggable)
       ; (bind-in! e [mid .-minBoundX] (when draggable 10))
       ; (bind-in! e [mid .-minBoundY] (when draggable 10))
       ; (bind-in! e [mid .-maxBoundX] (when draggable 500))
       ; (bind-in! e [mid .-maxBoundY] (when draggable 500))
...
(defn file-field [ctor]
  "types.  a list of file extensions, mime-types orheight of the font's capital letters, which may be expressed through
   - file extension. 
   - mime-type.
   - audio/*
   - video/*
   - image/*"
  (fn [{:keys [src prompt mime-types] :as attrs} elems]
    {:pre [(v/types?]} ;accept [".jpg" ".png" "audio/*" "video/*" "image/*" "application/ogg"]
    (with-let [e (ctor attrs elems)]
      (inherit (out e)
...
(defn files-field [ctor]
  (fn [{:keys [src prompt accept] :as attrs} elems]
