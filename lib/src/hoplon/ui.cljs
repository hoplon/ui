...
(defn gutter [ctor]
  ...
  (fn [{:keys [g gh gv] :as attrs} elems]
    ...
    (let [mh (cell= (hoplon.ui.attrs// (or gh g)  "2"))
          mv (cell= (hoplon.ui.attrs// (or gv g)  "2"))
          ph (cell= (hoplon.ui.attrs// (or gh g) "-2"))
          pv (cell= (hoplon.ui.attrs// (or gv g) "-2"))]
...
(def leaf (comp shadow round border pad size listable text fillable transform pointable))
