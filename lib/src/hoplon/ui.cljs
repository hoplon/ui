(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.attrs :refer [r ratio? calc? ->attr ->font font extrinsic explicit]]
...
(defn text [ctor]
  ...
  ts spacing. the additional space to be added between the letters.
   - <Pixel>
   - <Ratio>
  ...
  (fn [{:keys [t ts th tf tc td tx tk tr tm ta] :as attrs} elems]
    {:pre [(v/sizes? t) (v/spacings? ts th) (v/fonts? tf) (v/colors? tc)
           (v/decorations? td) (v/capitalizes? tx) (v/kernings? tk)
           (v/renderings? tr) (v/smoothings? tm) (v/adjusts? ta)]}
    (with-let [e (ctor attrs elems)]
      (when tf
        (hoplon.core/with-dom (out e)
          (let [ptr (atom nil)]
            (add-watch ptr (gensym)
              (fn [_ _ or nr]
                (when-not (cell? or)
                  (bind-with! #(swap! nr (partial apply assoc-in) (->font %)) tf))))
            (.dispatchEvent (out e) (js/CustomEvent. "bind" #js{:bubbles true :detail ptr})))))
    ...
    (let [get-hash   #(if (= (get js/location.hash 0) "#") (subs js/location.hash 1) js/location.hash)
      ...
      (with-let [e (ctor attrs elems)]
        ...
        (h/head
          ...
          (let [+fonts+ (cell {})]
            (.addEventListener (in e) "bind" #(reset! (.-detail %) +fonts+) true)
