(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.utils :refer [clean dash name descend? current target font-face FileList->vec with-ready]]
...
(defn bind! [source f]
  "accepts a cell and some function that create a side effect."
  (f @(add-watch source (gensym) #(f %4))))

(defn bind-with! [f value]
  (if (cell? value) (bind! value f) (f value)))
...
(defn size [ctor] ; _e_xtent
  ...
  (fn [{:keys [s sh sv sh- sh+ scroll overflowv] :as attrs} elems]
    ...
    (binding [*sx* (or sh s)
     ...
     (with-let [e (ctor attrs elems)]
      ;; when ratio subtract (- size margin)
       ...
       (bind-in! e [mid .-style .-overflowY] overflowv) ;; todo: find better solution
...
(defn pointable [ctor] ; _p_ointer
  ...
  (fn [{:keys [m me ms enter over move leave down down-off up up-off click click-dbl click-aux click-off ctx-menu select wheel drag-enter drag-over drag-leave drag-start drag drag-end drop] out* :out :as attrs} elems]
    {:pre [(v/cursors? m) (v/events? me) (v/callbacks? enter over move leave out* down down-off up up-off click click-dbl click-aux click-off ctx-menu select wheel drag-enter drag-over drag-leave drag-start drag drag-end drop)]}
    (with-let [e (ctor attrs elems)]
      (let [ms (cell= (if ms :text :none))
        ...
        (when enter      (.addEventListener (mid e) "mouseenter"  enter)) ;; todo: remove listeners when set by cells
        ...
        (when down-off   (.addEventListener js/document "mousedown"    #(when-not (descend? (mid e) (target %)) (down-off %)))) ;;todo: calculate off via mouse position
        (when up         (.addEventListener (mid e) "mouseup"     up))
        (when up-off     (.addEventListener js/document "mouseup"      #(when-not (descend? (mid e) (target %)) (up-off %))))
        (when click      (.addEventListener (mid e) "click"       click))
        (when click-dbl  (.addEventListener (mid e) "dblclick"    click-dbl))
        (when click-off  (.addEventListener js/document "click"   #(when-not (descend? (mid e) (target %)) (click-off %))))
        (when click-aux  (.addEventListener (mid e) "auxclick"    click-aux))
...
(defn foreable [ctor]
  (fn [{:keys [x y v] :as attrs} elems]
    (with-let [e (ctor attrs elems)]
      (bind-in! e [out .-style .-display]   (cell= (if v "table-cell" "none")))
      (bind-in! e [out .-style .-position]  "absolute")
      (bind-in! e [out .-style .-zIndex]    "2147483647")
      (bind-in! e [mid .-style .-display]   "block")
      (with-ready (out e)
...
(def fore    (-> h/div      box                                     node foreable   parse-args))
...
(def advance
  "generate up to 60 interpolated values per second."
  [sink start stop beg end easing]
  (let [start   (or start 0)
        stop    (or stop  1000)
        easing  (or easing i/linear)
        ease    (easing [start stop] [beg end])
        init    (atom nil)
        advance (fn advance [time]
                 (when-not @init (reset! init time))
                 (let [elapsed (- time @init)]
                   (reset! sink (ease elapsed))
                   (if (< elapsed stop)
                       (.requestAnimationFrame js/window advance)
                       (do (reset! init nil) (reset! sink end)))))]
    (.requestAnimationFrame js/window advance)))
...
(defn a=
  "animatate between a sequence of numeric values delimited by durations and
   interpolators that prescribe the time and easing between each pair of values
   in the sequence, returnng a cell that generates up to 60 intermediate numbers
   per second.

    (a= 300 1000 i/quadratic 600 1000 i/quadratic 800)

   number. any numerical value that should be animated; may be a cell.

   duration. the elapsed time between the values on the left and right, in
   milliseconds.

  interpolator. the function that will be called to advance the numerical
  values over the duration.

  repeat. duration in ms between animation sequences. defaults to none.

  note that a= can also be used with a transition function, to animate into the
  initial value supplied by a transition's source cell, for example. to make a
  bar in a graph animate from the bottom into its initial position:

    (elem :sh 20 :sv (a= 0 1000 (t= (cell= <value>) 1000 i/quadratic)) ...)"
  [beg duration interpolator end & rest]
  (with-let [sink (cell nil)]
    (h/with-init!
      (apply mapv vector (partition 4 3 (conj rest beg duration interpolator end)))
      (doseq [[start beg stop end] (reduce-kv (fn [xs dur val] (cons [x (=)(ffirst xs) xs [(+ (first (last xs)) dur) val (partition 2 (conj xs x 0))]])))]
        (prn :start start :stop stop :beg beg :end end)
        (advance sink beg (+ stend start stop (some #(when (fn? %) %) xs)))))))

(defn b=
  "break between the vertical or horizontal screen sizes."
  [x & xs]
  (with-let [v (cell nil)]
...
(defn state= [source f]
  "accepts a source and a function that accepts the sink and source, respectively,
   as arguments.. a cell where
   updates are triggered by actions other than the source cell, such as a timer
   used for animations or some actions.  returns a cell that gets updated by the
   function."
  (with-let [sink (cell @source)]
    (add-watch source (gensym) #(swap! sink f %4)))) ;; new

(defn t=
  "transition.  animate the value between two states derived from the cell.

   source.  the cell from which old and new values are obtained.
    (elem :sv (t= (cell= <value>)) ...)"
  [source & [duration interpolator]]
  (state= source #(advance % 0 duration %2 %3 interpolator)))
  ; (with-let [sink (cell @source)]
  ;   (bind! source #(advance sink 0 duration %3 %4 interpolator))))
