...
(defn fieldable [ctor]
  ...
  (fn [{:keys [src] :as attrs} elems]
    (with-let [e (ctor attrs elems)]
      ...
      (when (lens? src)
        (.addEventListener (in e) "change" #(reset! src (not-empty (.-value (in e)))))
        (.addEventListener (in e) "keyup"  #(reset! src (not-empty (.-value (in e)))))))))
    ...
    (with-let [e (ctor attrs elems)]
      (inherit (out e)
        (let [i (.appendChild (mid e) (.createElement js/document "input"))]
          ...
          #_(bind-in! i [.-value] src)
          (when (lens? src)
            (.addEventListener i "change"
              #(reset! src (when (not-empty (.-value i)) {:name (.-name (.item (.-files i) 0)) :data (.item (.-files i) 0)})))))))))
...
(def files   (-> h/div      box                     file-field      node            parse-args))
...
(defn t
  ...
  (let [duration (or duration 1000)
    ...
    (with-let [sink (cell nil)]
      (set-cell!= sink source)
      (add-watch source (gensym)
        #(let [ease  (easing [0 duration] [@sink @source])
               start (atom nil)
               step  (fn step [time]
  ...
  (let [pl (or p ph pl 0)
    ...
    (svg :view-box [0 0 (+ bh pl pr) (+ bv pt pb)] (dissoc attrs :b :bh :bv :p :ph :pv :pl :pr :pt :pb :src :k :kc)
      (s/g :transform (translate pl (+ bv pt))
        (s/path :d (cell= (do (line-path src))) :stroke-width (cell= (->attr (or k 0))) :stroke (cell= (->attr (or kc "black"))) :stroke-linecap "round")))))
