(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.utils :refer [clean dash name descend? current target FileList->vec]]
...
(defn pointable [ctor] ; _p_ointer
  ...
  (fn [{:keys [m me ms enter over move leave down up click click-dbl click-aux click-off ctx-menu select wheel drag-enter drag-over drag-leave drag-start drag drag-end drop] out* :out :as attrs} elems]
    {:pre [(v/cursors? m) (v/events? me) (v/callbacks? enter over move leave out* down up click click-dbl click-aux click-off ctx-menu select wheel drag-enter drag-over drag-leave drag-start drag drag-end drop)]}
    (with-let [e (ctor attrs elems)]
