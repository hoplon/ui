...
(defn text [ctor]
  ...
  (fn [{:keys [t ts th tf tc td tx tk tr tm ta] :as attrs} elems]
    {:pre [(v/sizes? t) (v/spacings? ts th) #_(v/fonts? tf) (v/colors? tc)
           (v/decorations? td) (v/capitalizes? tx) (v/kernings? tk)
           (v/renderings? tr) (v/smoothings? tm) (v/adjusts? ta)]}
    (with-let [e (ctor attrs elems)]
      #_(bind-in! e [in .-style .-fontSynthesis] (cell= (if ts ts "none")))
      (bind-in! e [in .-style .-fontSize]               t)
      (bind-in! e [in .-style .-letterSpacing]          ts)
      (bind-in! e [in .-style .-lineHeight]             th)
      (bind-in! e [in .-style .-fontFamily]             tf)
      (bind-in! e [in .-style .-color]                  tc)
      #_(bind-in! e [in .-style .-textDecoration]         td)
...
(defn font! [& args]
  (with-let [family (str (gensym "font"))]
    (let [[srcs kwargs] (split-with (comp not keyword?) args)
          {:keys [width weight slope]} (apply hash-map kwargs)
          loc    #(str "local('" % "')")
          url    #(str "url('" %1 "') format('" %2 "')")
          src    (join "," (mapv #(if (string? %) (loc %) (url (:url %) (:format %))) srcs))
          props  {"font-family"   family
                  "font-stretch"  (name (v/stretch? width))
                  "font-weight"   (name (v/weight?  weight))
                  "font-style"    (name (v/style?   slope))
                  "src"           src}
          font   (h/style (str "@font-face{" (apply str (mapcat (fn [[k v]] (str k ":" v ";")) (clean props))) "}"))]
      (.appendChild (.-head js/document) font) ;; load fonts eagerly
      (.appendChild (.-body js/document) (h/p :css/font-family family :css/font-color"0xFFF"))
      (h/with-init! (.appendChild (.-head js/document) font))
      (h/on-page-load (h/with-timeout 100 (prn :called) (.appendChild (.-head js/document) font))))))
