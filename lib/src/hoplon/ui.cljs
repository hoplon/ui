(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.utils  :refer [clean dash name pad descend? current target font-face FileList->vec with-ready]]
...
(defn ->font [x]
  (cond (nil? x) ""
        (map? x) (str (first (:path x)) (when-let [g (:generic x)] (str "," (name g))))))
  ...
  (fn [{:keys [s sh sv sh- sh+ scroll overflowv] :as attrs} elems]
    ...
    (binding [*sx* (or sh s)
      ...
      (with-let [e (ctor attrs elems)]
        ...
        (bind-in! e [mid .-style .-height]    (cell= (explicit  *sy*))) ;; cond fixes firefox vertical centering
...
(defn fillable [ctor] ; _f_ill
  ...
  (fn [{:keys [c o v] :as attrs} elems]
    ...
    (with-let [e (ctor attrs elems)]
      (bind-in! e [mid .-style .-background] (cell= (->background c)))
...
(defn border [ctor] ;_s_troke
  ...
  (fn [{:keys [b bh bv bl br bt bb bc bch bcv bcl bcr bct bcb] :as attrs} elems]
    ...
    (with-let [e (ctor attrs elems)]
      ...
      (bind-in! e [mid .-style .-borderLeftColor]   (cell= (->color (or bcl bch bc))))
      (bind-in! e [mid .-style .-borderRightColor]  (cell= (->color (or bcr bch bc))))
      (bind-in! e [mid .-style .-borderTopColor]    (cell= (->color (or bct bcv bc))))
      (bind-in! e [mid .-style .-borderBottomColor] (cell= (->color (or bcb bcv bc)))))))
...
(defn text [ctor]
  ...
  (fn [{:keys [t tw th tf tc td ts tx tk tr tm ta] :as attrs} elems]
    ...
    (with-let [e (ctor attrs elems)]
      (when tf
        (inherit (out e)
          (bind-with! (fn [v] (swap! +fonts+ assoc-in (:path tf) (:sources tf))) tf)))
...
(def view    (-> h/div      box                                     node            parse-args))
...
(hoplon.core/defelem path
  ...
  [{:keys [b bh bv p ph pv pl pr pt pb src k kc] :as attrs} elems]
  (let [pl (or p ph pl 0)
        pr (or p ph pr 0)
        pt (or p pv pt 0)
        pb (or p pv pb 0)
